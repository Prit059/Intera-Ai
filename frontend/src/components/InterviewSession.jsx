...
const VAPI_API_KEY = import.meta.env.VAPI_API_KEY;
const VAPI_WORKFLOW_ID = import.meta.env.VAPI_WORKFLOW_ID;
...
const InterviewSession = ({ sessionId, resumeText, onEnd }) => {
  ...
  useEffect(() => {
    ...
    const onHide = () => {
      if (document.hidden) {
        ...
        axios.post('http://localhost:5000/api/interview-session/update', {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      axios.post('http://localhost:5000/api/interview-session/update', { sessionId, metrics: tickMetrics })
        .catch(console.error);
    }, 1000);
  ...
  const startCall = useCallback(async () => {
    ...
    // Automatically start hidden audio recording
    try {
      ...
      console.warn("Failed to initialize background audio recording:", e);
    }
...
7. Once all 4 questions have been answered, conclude the interview gracefully.`
  ...
  const handleEnd = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/interview-session/end', {
        sessionId, vapiAnalysis: finalAnalysis, transcript,
      });
      onEnd({ ...response.data.evaluation, vapiAnalysis: finalAnalysis, transcript, audioUrl });
    } catch (err) {
